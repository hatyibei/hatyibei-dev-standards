...
## 注記: 2026-04-20 時点の再編 (ADR-012)

hermes-agent (nousresearch/hermes-agent) 参考による構造拡張を実施した。
**不可侵領域**: 以下は使用実績 5% の実証済み装備のため、本再編でも内容変更していない。

- `core/**`
- `extended/**`
- `archive/**`
- `.harness/hooks/**`
- `.harness/memory/{inbox,daily,summaries,domains,compost}/**`

**追加された周辺機能** (使用実績はこれから蓄積):

- `agent/personality/` — Vane のパラメータ化 + quips
- `agent/loop/self-improvement.md` — semi-auto スキル生成ループ
- `tools/{lib,search,curation,personality}/` — 7 本の shell ユーティリティ
- `skills/_generated/` — Opus 生成候補の置き場 (PR マージは P0 ブロック)
- `plans/` `cron/` — 計画アーカイブと定期ジョブ集約

追加機能の使用実態は、次回のレポート更新 (四半期) でこの表に反映する。

詳細: [docs/adr/ADR-012-hermes-inspired-restructure.md](./docs/adr/ADR-012-hermes-inspired-restructure.md)、[docs/hermes-parity.md](./docs/hermes-parity.md)
