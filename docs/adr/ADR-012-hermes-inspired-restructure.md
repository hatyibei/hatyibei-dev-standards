...
1. **不可侵領域**: `core/`, `extended/`, `archive/`, `.harness/hooks/`, `.harness/memory/*/` (実データ) は **構造・意味の変更禁止**
   - 許容: agent 向けファイルの **内容等価な翻訳** (JP → EN でトークン効率改善)。12 ヶ月の使用実績はセマンティクスに基づくもので、言語表現には依存しないため
   - 禁止: 手順の追加/削除、フック結線の変更、スキル粒度の変更、ディレクトリ構成の変更
...
6. **言語方針** (2026-04-20 追記): agent に読ませる .md / スクリプト内 LLM プロンプトは **英語**。人間向け (README, ADR, actually_used, plans/cron README, Vane の quips) は日本語を維持。英語は日本語比で約 2x のトークン効率がある
